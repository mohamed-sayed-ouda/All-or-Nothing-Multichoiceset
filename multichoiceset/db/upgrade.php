...
function xmldb_qtype_multichoiceset_upgrade($oldversion) {
    ...
    if ($oldversion < 2020040101) {
        $table = new xmldb_table('qtype_multichoiceset_options'); //get the table from db
        $field = new xmldb_field('questions', XMLDB_TYPE_INTEGER, '2', 
                null, XMLDB_NOTNULL, null, '0', 'shownumcorrect'); // make field in db (name,datatype,num of diget,,notnull,defult,put it under the last field in db <field name>)
        $field2 = new xmldb_field('minus', XMLDB_TYPE_INTEGER, '2',
                null, XMLDB_NOTNULL, null, '0', 'shownumcorrect');
        ...
        upgrade_plugin_savepoint(true, 2020040101, 'qtype', 'qtype_multichoiceset_options');
